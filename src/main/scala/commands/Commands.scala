...
case class Commands(userID : Int) {
  ...
  def createPoll(params: Try[List[String]]): String = {
    ...
    val anonymous: Try[Boolean] = Try(
      ...
      else ParamsParser.parseAll(ParamsParser.anonymous, params.get(1)).get
    )

    val viewType: Try[String] = Try(
      if (Try(params.get(2)).isFailure) "afterstop"
      else ParamsParser.parseAll(ParamsParser.view_type, params.get(2)).get
    )

    val startTime: Try[String] = Try(
      if (Try(params.get(3)).isFailure) getDateTime(new Date().toString)
      else ParamsParser.parseAll(ParamsParser.time, params.get(3)).get
    )

    val stopTime: Try[String] = Try(
      if (Try(params.get(4)).isFailure) "not set"
      else ParamsParser.parseAll(ParamsParser.time, params.get(4)).get
    )
  ...
  def addQuestion(params: Try[List[String]],
    ...
    val qtype = Try(if (Try(params.get.last).isFailure) "open"
      else ParamsParser.parseAll(ParamsParser.qtype, params.get.last).get
    )
  ...
  def answer(id : Try[List[String]]): Unit = {
    ???
  }
}
